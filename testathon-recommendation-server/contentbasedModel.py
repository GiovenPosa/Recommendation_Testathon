...
async def contentBasedRec(user_profile):
...
# === Scoring for selected (for legacy normalized 'score') ===
  ...
  for s, trip in selected_scored:
      # Legacy score: min–max within the selected set (0..1)
      score_norm = (s - sel_min) / sel_rng

      # Probability-like confidence using ABSOLUTE baseline (vs best in ALL candidates)
